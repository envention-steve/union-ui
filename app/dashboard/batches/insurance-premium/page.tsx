...
interface InsurancePremiumBatchListItem {
  id: number;
  start_date?: string;
  end_date?: string;
  posted?: boolean;
...
export default function InsurancePremiumBatchesPage() {
  const router = useRouter();
  const [batches, setBatches] = useState<InsurancePremiumBatchListItem[]>([]);
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await backendApiClient.insurancePremiumBatches.list({
          page: currentPage,
          limit: itemsPerPage,
          startDate: startDate || undefined,
          endDate: endDate || undefined,
        });
        const paginated = response as PaginatedResponse<InsurancePremiumBatchListItem>;
        setBatches(paginated.items ?? []);
        setTotalBatches(paginated.total ?? 0);
      } catch (fetchError) {
        console.error('Failed to load insurance premium batches', fetchError);
        setError('Failed to load insurance premium batches. Please try again.');
        setBatches([]);
        setTotalBatches(0);
      } finally {
        setLoading(false);
  ...
  const handleDeleteBatch = async (id: number) => {
    ...
    try {
      await backendApiClient.insurancePremiumBatches.delete?.(String(id));
      triggerReload();
    } catch (deleteError) {
      console.error('Failed to delete insurance premium batch', deleteError);
      setError('Failed to delete insurance premium batch. Please try again.');
    }
  };
  ...
  const deriveStatusLabel = (batch: InsurancePremiumBatchListItem) => {
    if (batch.suspended) {
      return 'Suspended';
    }
  ...
  const statusBadgeVariant = (batch: InsurancePremiumBatchListItem) => {
    if (batch.suspended) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-union-900">Insurance Premiums</h1>
          <p className="text-muted-foreground">Review insurance premium batches and manage their lifecycle.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Insurance Premium Batches</CardTitle>
          <CardDescription>
            {loading
              ? 'Loading batches...'
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={3} className="h-24 text-center text-muted-foreground">
                      {filterApplied
                        ? 'No insurance premium batches found for the selected filters.'
                        : 'No insurance premium batches found.'}
                    </TableCell>
                  </TableRow>
                ) : (
                  batches.map((batch) => (
